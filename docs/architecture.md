...
- **Placement controls:** cursor-to-floor targeting, Q/E yaw rotation, left click to confirm, and Escape to cancel. Keyboard and right-mouse movement remain active, so reach and validity update from the player's current position. Exact key bindings and angles are tuning choices; identity-preserving return to the world is required.
