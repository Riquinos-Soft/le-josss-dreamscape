...
func _initialize() -> void:
	...
	check(Direction.from_view(Vector2(1, -1), Basis.IDENTITY), Vector3(1, 0, -1).normalized(), "diagonal capped")
	check(Direction.from_view(Vector2(0.25, 0), Basis.IDENTITY), Vector3(0.25, 0, 0), "analog magnitude")
	...
	check(Direction.from_view(Vector2(0, -1), pitched), Vector3.FORWARD, "pitch does not slow or lift movement")
	check(Direction.from_view(Vector2(0, -1), Basis(Vector3.RIGHT, -PI / 2.0)), Vector3.FORWARD, "overhead view")
